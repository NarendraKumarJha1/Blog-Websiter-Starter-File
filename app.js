...
app.get("/about",function(req, res){
  res.render("about",{aboutContent: homeAboutContent})
})
...
app.get("/post/:postTitle",function(req, res){
  ...
  for(i=0;i<posts.length;++i){
    if(_.lowerCase(req.params.postTitle)==_.lowerCase(posts[i].title)) {
      titleToRender=posts[i].title
      postToRender=posts[i].content
...
app.get("/post", function(req, res) {
  res.render("post",{titlesp:titleToRender,postsp:postToRender})
})

app.get("/contact",function(req, res) {
  res.render("contact", {contactContent:homeContactContent})
})
...
app.listen(1166, function() {
  console.log("Server started on port 1166")
})
